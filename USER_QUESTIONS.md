...
**R11.4 · Sun & Moon variants.** TCGdex has no variants for most Sun & Moon sets. Settr takes TCGplayer's printings per card (Normal, Holo, Reverse-Holo) and keeps them between syncs; a card TCGplayer doesn't list gets them from its rarity, basic Energy as *Normal* only (ADR-057). A Rare that only exists as a holo shows as *Holo Rare*.
- [x] ⭐ Like that.
...
- [x] ⭐ Not now; I add them when you ask.
- [ ] Add them now.
