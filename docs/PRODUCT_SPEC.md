...
- **Chinese data isn't in TCGdex** (its zh data stops before the Mega series). Decision (Q3.2/Q3.5, R2.3): **Simplified and Traditional Chinese copies are trackable from day one** on the M6a card list, with M6a's numbering.
  - *Simplified Chinese names* are derived from PokéAPI `zh-Hans` species names (+ `ex`, labeled *übersetzt*), plus hand-curated Trainer/Energy names. Settr doesn't use the SC dataset `duanxr/PTCG-CHS-Datasets` at all: its terms reserve consent for redistribution to the official owner or an authorized entity, not the maintainer (R2.8).
  - *Traditional Chinese names* come from `type-null/PTCG-database` (`data_tc`; its data licensing is to verify). The fallback is PokéAPI `zh-Hant` species names (*übersetzt*), and Trainer names are curated by hand (R2.3).
