...
> Entities → [`DATA_MODEL.md`](./DATA_MODEL.md). Items marked **⟶ Qn** depend on [`USER_QUESTIONS.md`](../USER_QUESTIONS.md).
...
This is a **mapping wizard**: upload a CSV, auto-detect the delimiter and encoding, map columns to Settr fields, then **match** rows to catalog items by set + number (fuzzy fallback on name) and show unmatched rows for manual resolution. Presets for known formats (e.g. Collectr, TCG Collector, Cardmarket stock exports) are added once sample files are available ⟶ **Q7.4**.
...
| **Backup reminder** | Pill + toast when `lastBackupAt` is older than N days (default 7) **and** there were changes since. The interval is configurable ⟶ **Q7.1** |
| **Change counter** | After 50 changes without a backup, a gentle reminder |
| **Auto-backup to folder** (I-15, Chromium desktop) | The user grants a directory once (the handle is stored in IndexedDB). Settr writes `settr-backup-latest.settr.json` and rotating dated copies (keeps the last 10) after changes (debounced 60 s) and on `visibilitychange: hidden`. Permission is re-requested per session when needed |
...
## 9. Future: multi-device sync without a backend (⟶ Q7.3)
