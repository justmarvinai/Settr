...
Allowed now: editing planning docs (`*.md`), answering questions, research, and (only if Marvin approves Q9.1) design mockups.

When permission is granted, update this block with the date and the approved scope (e.g. "M1 approved on 2026-10-01").

## What Settr is

Settr is a **local-first Pokémon TCG collection tracker** for **singles and sealed** products, in German (primary), English, Japanese and Chinese cards.

- **Prices are entered manually** (from Cardmarket) and turned into trends and P/L versus purchase price.
- It's a static SPA on **Vercel**, with **no backend**. All user data lives in **IndexedDB**, with full import/export.
- **v1 catalog = one expansion:** *30 Jahre / 30th Celebration* (EN/DE `30th` + `30th-c` + 8 energies; JP `M6a`), released 16 Sep 2026.
...
| Tokens, typography, motion, components | `docs/DESIGN_SYSTEM.md` |
| i18n, formatting, DE/EN glossary | `docs/I18N.md` |
...
| Open questions for Marvin | `USER_QUESTIONS.md` |
...
5. **No hard-coded UI strings.** Every string goes in Paraglide messages (DE + EN). German is the base locale, and the UI uses informal "du" (pending Q2.4).
...
8. **Privacy:** no analytics, trackers or third-party requests beyond approved image hosts (pending Q8.3/Q8.4).
