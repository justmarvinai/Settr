...
**Current phase: v1.x · YOUR SETS — the Mega Evolution series ✅ built, waiting for Marvin's check** (2026-09-24: "add the whole Mega Evolution Series"). Coding was approved on 2026-09-23 (Marvin: "You can start"). M1–M6 are merged into `main`: PR #1 (M1 + M2), PR #2 (M3), PR #3 (M4 + M5), PR #4 (M6, merged 2026-09-24). The Mega work is on `claude/great-edison-uri1z0` (restarted from `main` after PR #4); a PR is opened when Marvin asks. The tag `v1.0.0` goes on `main` once Marvin says go (R8.4). Vercel is connected. Open on Marvin's side: say go for the tag, add the `VERCEL_DEPLOY_HOOK` secret for the daily price guide, make `main` the default branch, run the Brave smoke test (`docs/QUALITY.md` §3.1) and the iPhone check (`USER_QUESTIONS.md` rounds 8–9, "To try"), answer rounds 8–9 when convenient.
